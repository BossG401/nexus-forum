  ...
  return (
    <div className="max-w-3xl animate-fade-in-up">
      ...
      <div className="mb-5 rounded-2xl border border-border bg-card p-5 shadow-sm">
        <div className="flex items-start gap-4">
          <Avatar className="h-18 w-18 ring-1 ring-border">
            <AvatarImage src={dbUser.image ?? "/avatars/default.png"} alt={dbUser.name ?? "召唤师"} />
          ...
          <div className="min-w-0 flex-1">
            <h1 className="text-xl font-bold tracking-tight text-foreground">{dbUser.name ?? "未知召唤师"}</h1>
            <div className="mt-1.5 flex flex-wrap items-center gap-2.5">
              {dbUser.lolRank && <span className={cn("text-sm", rankStyles[dbUser.lolRank] ?? "text-muted-foreground")}>{rankLabel(dbUser.lolRank)}</span>}
            ...
            <div className="mt-3 flex items-center gap-4 text-xs text-muted-foreground">
              <span className="flex items-center gap-1"><Hash size={13} />{mappedPosts.length} 篇帖子</span>
      ...
      <div className="mb-4 flex items-center gap-2">
        <h2 className="text-lg font-semibold tracking-tight text-foreground">我的帖子</h2>
      ...
      {mappedPosts.length > 0 ? (
        ...
        <div className="rounded-2xl border border-border bg-card p-10 text-center animate-fade-in">
          <p className="text-base font-semibold text-foreground">还没有帖子</p>
          <p className="mt-2 text-sm text-muted-foreground">你发布的帖子会显示在这里。</p>
          <Link href="/submit" className="mt-5 inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground transition-colors hover:bg-primary/90"><Swords size={15} />发布你的第一篇帖子</Link>
        </div>
      )}
    </div>
  )
